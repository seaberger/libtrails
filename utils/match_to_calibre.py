...
for row in cursor.fetchall():
    ...
    calibre_index[key].append(
        {"id": row["id"], "title": row["title"], "author_sort": row["author_sort"]}
    )
...
for ipad_book in ipad_books:
    ...
    if len(candidates) == 1:
        ...
        for c in candidates:
            if (
                normalize(c["author_sort"]) == ipad_author_norm
                or ipad_author_norm in normalize(c["author_sort"])
                or normalize(c["author_sort"]) in ipad_author_norm
            ):
                match = c
                break
        if not match:
