                ...
                if data:
                    ...
                    if "description" in book:
                        desc = (
                            ...
                            else book["description"].get("value")
                        )
                    ...
                    if work_key:
                        ...
                        with urllib.request.urlopen(work_url, timeout=10) as w_response:
                            ...
                            if "description" in work:
                                desc = (
                                    work["description"]
                                    if isinstance(work["description"], str)
                                    else work["description"].get("value")
                                )
...
for i, book in enumerate(missing):
    ...
    if result:
        ...
        if desc:
            cursor.execute(
                "UPDATE books SET description = ? WHERE id = ?", (clean_html(desc), book_id)
            )
            enriched += 1
        ...
        for subject in subjects[:5]:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO book_calibre_tags (book_id, tag_id) VALUES (?, ?)",
                (book_id, tag_id),
            )
