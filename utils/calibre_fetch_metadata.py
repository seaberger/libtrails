    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            if desc and len(desc) > 50:
                ...
                cursor.execute(
                    "UPDATE books SET description = ? WHERE id = ?", (clean_desc, book_id)
                )
                updated += 1
                ...
                if tags:
                    for tag in tags.split(","):
                        ...
                        if tag:
                            cursor.execute(
                                "INSERT OR IGNORE INTO calibre_tags (name) VALUES (?)", (tag,)
                            )
                            cursor.execute("SELECT id FROM calibre_tags WHERE name = ?", (tag,))
                            tag_id = cursor.fetchone()[0]
                            cursor.execute(
                                "INSERT OR IGNORE INTO book_calibre_tags VALUES (?, ?)",
                                (book_id, tag_id),
                            )
